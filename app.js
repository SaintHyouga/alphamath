...
app.all('/mbanking', (req, res) => {
  ...
  if(req.body.txtInput){
    ...
    if(codeSave=="1"){
        ...
        mysql.query(sql,[curItem],function(err,result){
            ...
            if(nRow>0){
                //update
                var sql = "update mathdb set itemContent = ?,itemTag= ? where itemId = ? and itemCreator=?";
                mysql.query(sql,[txtInput,itemTag,curItem,userId],(err,result)=>{
                  if(err) throw err;
              ...
              //insert
              var sql = "insert into mathdb (itemContent,itemTag,itemCreator) values ?";
  ...
  if(req.body.itemToEdit){
    ...
    if(req.body.toUndo){
      ...
      res.render('mbanking', { title: 'M-Banking',txtInput:txtInput,txtOutput:txtOutput,codeSave:codeSave,itemTag:itemTag,username:req.session.username,curItem:"0",userId:req.session.userId});
    }
  }
}
});
...
app.all('/qStudent', (req, res) => {
  ...
  mysql.query(sql,(err,result)=>{
    ...
    if(result.length== 0){
      res.render('qStudent',{ title: 'การนำเสนอ',txtOutput:0,username:req.session.username});
    }else{
      res.render('qStudent',{ title: 'การนำเสนอ',txtOutput:result[0].itemContent,username:req.session.username});
    }
...
//หา curPresent
 ...
 if(optSearch==1){
   var sql = "select itemId,itemContent,itemTag,itemCreator from mathdb where itemId= ?";
   mysql.query(sql,[curItem],(err,result)=>{
     if(err) throw err;
     console.log(result);
     res.render('qTeacher', { title: 'จัดการคำถาม',items:result,curItem:curItem,optSearch:optSearch,curPresent:curPresent,username:req.session.username,userId:req.session.userId});
   });
 }else{
   var sql = "select itemId,itemContent,itemTag,itemCreator from mathdb where itemTag like ?";
   mysql.query(sql,["%"+curItem+"%"],(err,result)=>{
     if(err) throw err;
     console.log(result);
    res.render('qTeacher', { title: 'จัดการคำถาม',items:result,curItem:curItem,optSearch:optSearch,curPresent:curPresent,username:req.session.username,userId:req.session.userId});
